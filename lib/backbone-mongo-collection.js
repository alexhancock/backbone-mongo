...
Backbone.MongoCollection = Backbone.Collection.extend({
  ...
  update: function(sel, mod){
    var $dynMod = {};

    // Build the mongo modifier with "attributes" prefixes
    // There is probably an easier way to do this!
    _.each(mod, function(obj, modOp){
      $dynMod[modOp] = {};

      _.each(obj, function(val, key){
        // TODO - Go third level down for $set's that have multiple properties
        $dynMod[modOp]['attributes.'+key] = val;
      });
    });

    this.store.update(sel, $dynMod);
  }
